...
grid on;
% set up system
s_y = 2; % sources at [0; +- s_y]
t_x  = 100;
t_y = 2; % targets at [t_x; +- t_y]
xe = [27.7;0]; % center point on first lens surface
ct = t_x - 2 * xe(1); % center thickness
% ct = 40;
alpha = 30*pi/180;
%
plot([0,t_x/2],s_y+[0,t_x/2*tan(alpha)],'--k');
%
n_air = 1;
%n_glass = 1.523;
n_glass = 1.8;
...
function PlotLocation(loc, n)
    [px, py] = loc.Points(n);
    plot(gca, px, py, 'k');
    scatter(gca,[px(1),px(end)],[py(1),py(end)],'k')
end